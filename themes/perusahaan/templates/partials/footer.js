/* ============================================================
   ...
   Bagian utama: kolom brand, tautan cepat, dan kontak ringkas.
...
module.exports = function footer(ctx) {
  ...
  var copyright =
    config.footerCopyright && String(config.footerCopyright).trim()
      ...
      : "&copy; " + year + " " + esc(config.author || config.title) + ".";

  var links = quickLinks(config, U, lib);

  // Kolom kontak ringkas: pakai data Kontak bila diisi, fallback ke email situs.
  var social = config.social || {};
  var contactItems = [];
  if (contact.phone) contactItems.push('<li><a href="tel:' + attr(contact.phone.replace(/\s+/g, "")) + '">' + esc(contact.phone) + "</a></li>");
  if (contact.email) contactItems.push('<li><a href="mailto:' + attr(contact.email) + '">' + esc(contact.email) + "</a></li>");
  else if (social.email) contactItems.push('<li><a href="mailto:' + attr(social.email) + '">' + esc(social.email) + "</a></li>");
  if (contact.address) contactItems.push("<li>" + esc(contact.address) + "</li>");
  if (!contactItems.length) contactItems.push('<li><a href="' + attr(U.url("/kontak/")) + '">Hubungi Kami</a></li>');
